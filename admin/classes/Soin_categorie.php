...
class Soin_categorie extends StorageManager {
	...
	public function load( $id, $debug=false ){
		...
		$requete = "SELECT * FROM `soin_categorie` WHERE id = " . $id ;
	...
	public function gererDonnees( $post, $debug=false ) {
		...
		$val[ "sous_titre" ] = addslashes( $post[ "sous_titre" ] );
	...
	public function ajouter( $value, $debug=false ) {
		...
		try {
			$sql = "INSERT INTO `soin_categorie`
				( `titre`, `sous_titre` )
				VALUES (
				'" . $value[ "titre" ] ."',
				'" . $value[ "sous_titre" ] ."'
	...
	public function modifier( $value, $debug=false ) {
		...
		try {
			$sql = "UPDATE .`soin_categorie` SET";
			$sql .= " `titre` = '" . $value[ "titre" ] . "', ";
			$sql .= " `sous_titre` = '" . $value[ "sous_titre" ] . "'";
	...
	public function supprimer( $id, $debug=false ) {
		...
		try {
			...
			$sql = "DELETE FROM `soin_categorie`";
			$sql .= " WHERE `id`=" . $id . ";";
	...
	function setChamp( $id=0, $champ, $valeur=0, $debug=false ) {
		...
		$requete = "UPDATE `soin_categorie` SET";
	...
	public function getListe( $tab=array(), $debug=false ) {
		...
		$requete = "SELECT " . $champ_souhaite . " FROM `soin_categorie`";
