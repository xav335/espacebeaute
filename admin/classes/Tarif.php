...
class Tarif extends StorageManager {
	...
	public function load( $id, $debug=false ){
		...
		$requete = "SELECT * FROM `tarif` WHERE id = " . $id ;
	...
	public function ajouter( $value, $debug=false ) {
		...
		try {
			$sql = "INSERT INTO `tarif`
				( `id_categorie`, `titre`, `texte`, `prix`, `image`, `online` )
				VALUES (
				'" . $value[ "id_categorie" ] . "',
				'" . $value[ "titre" ] ."',
				'" . $value[ "texte" ] ."',
	...
	public function getListe( $tab=array(), $debug=false ) {
		...
		$requete = "SELECT " . $champ_souhaite . " FROM `tarif`";
